...
def identify_perspectives(query: str, retrieved_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
                    ...
                    "perspectives": [
                        {
                            ...
                            "key_points": ["The documents contain various perspectives on the topic."],
                            "supporting_documents": list(range(1, len(retrieved_docs) + 1))
                        }
                    ],
...
def generate_synthesis(
    ...
    disagreements_text = "\n".join([f"- {disagreement}" for disagreement in disagreements])
...
def main():
    ...
    if "GOOGLE_API_KEY" in os.environ:
        ...
        if documents_by_source:
            ...
            for source, docs in documents_by_source.items():
                with st.expander(f"View Documents from {source} ({len(docs)} documents)"):
            ...
            query, search_clicked = create_query_input(
                "Enter your question:",
                button_text="Search Across Perspectives"
            )
            ...
            if search_clicked and query:
                ...
                with st.spinner("Identifying perspectives..."):
                    ...
                    steps.append({
                        ...
                        "content": {
                            "Perspectives Identified": len(perspective_analysis.get("perspectives", [])),
                            "Areas of Agreement": len(perspective_analysis.get("agreements", [])),
                            "Areas of Disagreement": len(perspective_analysis.get("disagreements", []))
                        },
                ...
                with st.spinner("Generating synthesized answer..."):
                    ...
                    # Generate synthesis
                    synthesis = generate_synthesis(query, retrieved_docs, perspective_analysis)
                ...
                if perspectives:
                    for i, perspective in enumerate(perspectives):
                        with st.expander(f"Perspective: {perspective.get('name', f'Perspective {i+1}')}"):
                            # Key points

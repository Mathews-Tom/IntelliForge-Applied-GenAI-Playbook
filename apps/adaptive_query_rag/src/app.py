...
def get_retrieval_strategy(query_type: QueryType, confidence_scores: Dict[str, float]) -> Dict[str, Any]:
    """
    ...
    if query_type == QueryType.FACTUAL:
        ...
        alpha = (
            normalized_scores.get("FACTUAL", 0) * 0.4 +  # Factual uses embedding but we blend
...
def handle_comparison_query(
    ...
    for item in items:
        ...
        item_docs = execute_retrieval_strategy(
            item_query,
            documents,
            {"method": strategy.get("method", "hybrid"), "top_k": 3}  # Fewer docs per item
        )
    ...
    original_docs = execute_retrieval_strategy(
        query,
        documents,
        {"method": strategy.get("method", "hybrid"), "top_k": 3}
    )
...
def main():
    ...
    with st.sidebar:
        ...
        override_strategy = st.checkbox("Override automatic strategy selection", value=False)
    ...
    if "GOOGLE_API_KEY" in os.environ:
        ...
        if documents_df is not None:
            ...
            query, search_clicked = create_query_input(
                "Enter your question:",
                button_text="Search with Adaptive Strategy"
            )
            ...
            if search_clicked and query:
                ...
                with st.spinner("Analyzing query type..."):
                    ...
                    steps.append({
                        ...
                        "content": {
                            ...
                            "Confidence Scores": {k: f"{v:.2f}" for k, v in confidence_scores.items()},
                ...
                with st.spinner("Retrieving documents..."):
                    ...
                    if query_type == QueryType.COMPARISON and not override_strategy:
                        retrieved_docs = handle_comparison_query(query, documents, strategy)
                        retrieval_method = "Comparison-specific retrieval"
                    else:
                        # Standard retrieval
                        retrieved_docs = execute_retrieval_strategy(query, documents, strategy)
                        retrieval_method = f"{strategy['method'].capitalize()} retrieval"

...
def generate_answer(
    query: str, static_context: str, tool_results: Dict[str, Any]
) -> str:
    """
        ...
        if result.get("success", False) and result.get("data"):
            for stock_data in result["data"]:
                if "error" in stock_data:
                    tool_results_text += (
                        f"- {stock_data['ticker']}: {stock_data['error']}\n"
                    )
                else:
        ...
        if result.get("success", False) and result.get("data"):
            for weather_data in result["data"]:
                if "error" in weather_data:
                    tool_results_text += (
                        f"- {weather_data['location']}: {weather_data['error']}\n"
                    )
                else:
                    ...
                    tool_results_text += (
                        f"- {weather_data['location']}: {temp}{unit}, {conditions}, "
                    )
    ...
    if "web_search_tool" in tool_results:
        ...
        if result.get("success", False) and result.get("data"):
            ...
            tool_results_text += (
                f"- {result.get('message', 'No search results available')}\n"
            )
        ...
        if documents_df is not None:
            ...
            with st.expander("View Static Documents"):
                for i, doc in enumerate(documents):
                    ...
                    category = (
                        documents_df["category"][i]
                        if "category" in documents_df.columns
                        else "Unknown"
                    )
            ...
            query, search_clicked = create_query_input(
                "Enter your question:", button_text="Search with Tools"
            )
            ...
            if search_clicked and query:
                    ...
                    steps.append(
                        {
                            ...
                            "content": {
                                "Stock Tool Required": "Yes"
                                if required_tools.get("stock_tool", False)
                                else "No",
                                "Weather Tool Required": "Yes"
                                if required_tools.get("weather_tool", False)
                                else "No",
                                "Web Search Tool Required": "Yes"
                                if required_tools.get("web_search_tool", False)
                                else "No",
                                "Explanation": required_tools.get(
                ...
                tool_results = {}
                if any(
                    required_tools.get(tool, False)
                    for tool in ["stock_tool", "weather_tool", "web_search_tool"]
                ):
                        ...
                        for tool_name, result in tool_results.items():
                            if result.get("success", False):
                                tool_content[tool_name] = (
                                    f"Success: {result.get('message', 'No message')}"
                                )
                            else:
                                tool_content[tool_name] = (
                                    f"Failed: {result.get('message', 'No message')}"
                                )
                            ...
                            if result.get("success", False) and result.get("data"):
                                for stock_data in result["data"]:
                                    if "error" in stock_data:
                                        ...
                                        with col2:
                                            st.markdown(
                                                f"**{stock_data['price']} {stock_data['currency']}**"
                                            )
                                        with col3:
                                            change = stock_data["change_percent"]
                                            if change >= 0:
                                                st.markdown(
                                                    f"<span style='color:green'>+{change}%</span>",
                                                    unsafe_allow_html=True,
                                                )
                                            else:
                                                st.markdown(
                                                    f"<span style='color:red'>{change}%</span>",
                                                    unsafe_allow_html=True,
                                                )
                                        ...
                                        st.markdown(
                                            f"Last Updated: {stock_data.get('timestamp', 'N/A')}"
                                        )
                                        st.markdown("---")
                            else:
                                st.warning(
                                    result.get("message", "No stock data available")
                                )
                                ...
                                for weather_data in result["data"]:
                                    if "error" in weather_data:
                                        ...
                                        with col1:
                                            ...
                                            st.markdown(
                                                f"Feels like: {weather_data['temperature']['feels_like']}{unit}"
                                            )
                                            st.markdown(
                                                f"Min/Max: {weather_data['temperature']['min']}{unit} / {weather_data['temperature']['max']}{unit}"
                                            )
                                        with col2:
                                            st.markdown(
                                            ...
                                            st.markdown(
                                                f"Wind: {weather_data['wind']['speed']} {weather_data['wind']['unit']} {weather_data['wind']['direction']}"
                                            )
                                        ...
                                        st.markdown(
                                            f"Last Updated: {weather_data['timestamp']}"
                                        )
                                        st.markdown("---")
                            else:
                                st.warning(
                                    result.get("message", "No weather data available")
                                )
                        ...
                        with st.expander("Web Search Tool Results", expanded=True):
                            ...
                            if result.get("success", False) and result.get("data"):
                                for search_result in result["data"]:
                                    ...
                                    st.markdown(
                                        f"Date: {search_result.get('date', 'Unknown')}"
                                    )
                                    st.markdown(f"{search_result['snippet']}")
                                    if search_result.get("link"):
                                        st.markdown(
                                            f"[Read more]({search_result['link']})"
                                        )
                                    st.markdown("---")
                            else:
                                st.warning(
                                    result.get("message", "No search results available")
                                )

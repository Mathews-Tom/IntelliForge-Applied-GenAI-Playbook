...
from core.utils.evaluation import evaluate_answer_faithfulness, evaluate_retrieval_relevance
    ...
    try:
        ...
        if response:
            ...
            try:
                ...
                return {
                    "document_scores": [
                        {"doc_id": i+1, "score": 5, "issues": ["Failed to parse evaluation"]} 
        ...
        if documents_df is not None:
            ...
            with st.expander("View Documents"):
                for i, doc in enumerate(documents):
                    ...
                    source = documents_df["source"][i] if "source" in documents_df.columns else "Unknown"
            ...
            query, search_clicked = create_query_input(
                "Enter your question:",
                button_text="Search with Reflection"
            )
            ...
            if search_clicked and query:
                ...
                with st.spinner("Evaluating retrieval quality..."):
                    ...
                    if evaluation_result and "document_scores" in evaluation_result:
                        scores = [item.get("score", 0) for item in evaluation_result["document_scores"]]
                    ...
                    steps.append({
                        ...
                        "content": {
                            ...
                            "Sufficient": "Yes" if evaluation_result.get("overall", {}).get("sufficient", False) else "No",
                            "Missing Information": evaluation_result.get("overall", {}).get("missing_information", "None"),
                            "Suggested Reformulation": evaluation_result.get("overall", {}).get("query_reformulation", "None")
                        },
                ...
                if avg_score < reflection_threshold or not evaluation_result.get("overall", {}).get("sufficient", False):
                    with st.spinner("Performing re-retrieval with reformulated query..."):
                        start_time = time.time()
                        ...
                        if reformulated_query != query:
                            ...
                            re_retrieved_docs = embedding_retrieval(reformulated_query, documents, top_k=top_k)
                            
                            # Rerank all documents
                            all_docs = list(set([doc for doc, _ in retrieved_docs] + [doc for doc, _ in re_retrieved_docs]))
                            ...
                            steps.append({
                                ...
                                "content": {
                                    ...
                                    "Final Documents After Reranking": len(final_docs)
                                },
                ...
                with st.spinner("Evaluating answer faithfulness..."):
                    ...
                    faithfulness_result = evaluate_answer_faithfulness(draft_answer, context, query)
                    ...
                    steps.append({
                        ...
                        "content": {
                            ...
                            "Explanation": faithfulness_result.get("explanation", "No explanation provided"),
                            "Issues": faithfulness_result.get("issues", ["None identified"])
                        },
                ...
                if faithfulness_result.get("score", 10) < faithfulness_threshold:
                    with st.spinner("Refining answer..."):
                        ...
                        # Refine answer
                        final_answer = refine_answer(query, context, draft_answer, faithfulness_result)

...
def create_sidebar(title: str, description: str, 
                  api_key_field: bool = True) -> None:
    """
...
def display_documents(documents: List[Dict[str, Any]], 
    ...
    display_content = st.expander("Retrieved Documents", expanded=expand) if expand else st
    
    with display_content:
        st.info(f"Displaying {min(len(documents), max_docs)} of {len(documents)} documents.")
...
def file_uploader(label: str = "Upload your documents", 
...
def create_query_input(label: str = "Enter your query:",
                      height: int = 100,
...
def timed_operation(operation: Callable, 
                   description: str) -> Tuple[Any, float]:
    """

...
# Embedding functions
def get_embeddings(
    texts: List[str], cache: Optional[Dict[str, np.ndarray]] = None
) -> Dict[str, np.ndarray]:
    """
...
# Retrieval functions
def bm25_retrieval(
    query: str, documents: List[Union[str, Dict[str, Any]]], top_k: int = 5
) -> List[Tuple[Union[str, Dict[str, Any]], float]]:
    """
...
def hybrid_retrieval(
    ...
    embedding_results = embedding_retrieval(
        query, documents, embeddings, len(documents)
    )
